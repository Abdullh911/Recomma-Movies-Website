...
import { addComment } from '../recommaUtils/backendCalls';
import { useNavigate } from 'react-router-dom';

const AddCommentInput = ({ id, onCommentAdded }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await addComment(id, userData.email, userData.username, content);
            cmnt.current.value = '';
            setContent('');
            // Notify parent component (MoviePage) that a comment has been added
            if (onCommentAdded) {
                onCommentAdded();
            }
        } catch (error) {
